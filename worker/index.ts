...
import { appendRecord, cancelBooking, classifyStudent, createBooking, getBootstrap, rescheduleBooking, updateStudentProfile, type GoogleEnv } from "./google";
...
const worker = {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (url.pathname === "/api/students/profile" && request.method === "PATCH") {
      try { return Response.json(await updateStudentProfile(env, await request.json() as Record<string, unknown>)); }
      catch (error) { return Response.json({ error: error instanceof Error ? error.message : "學員資料儲存失敗" }, { status: 500 }); }
    }
